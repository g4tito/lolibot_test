...
`.trim()
    ...
    switch (type) {
        case 'comun':
            switch (jumlah) {
                    let _cm = `${Math.floor(Math.random() * 50 * jumlah)}`.trim()
                    let _cc = `${Math.floor(Math.random() * 2 * jumlah)}`.trim()
                    let _cp = `${Math.floor(Math.random() * 1 * jumlah)}`.trim()
                    let _ce = `${Math.floor(Math.random() * 100 * jumlah)}`.trim()
                    let _cu = `${Math.floor(Math.random() * 1 * jumlah)}`.trim()
                    let cm = (_cm * 1)
                    let cc = (_cc * 1)
                    let cp = (_cp * 1)
                    let ce = (_ce * 1)
...
`.trim()
        ...
        case 'normal':
            switch (jumlah) {
                    let _ud = `${Math.floor(Math.random() * 2 * jumlah)}`.trim()
                    let _ue = `${Math.floor(Math.random() * 100 * jumlah)}`.trim()
                    let _um = `${Math.floor(Math.random() * 150 * jumlah)}`.trim()
                    let _up = `${Math.floor(Math.random() * 2 * jumlah)}`.trim()
                    let _umc = `${Math.floor(Math.random() * 1 * jumlah)}`.trim()
                    let _uu = `${Math.floor(Math.random() * 2 * jumlah)}`.trim()
                    let _uc = `${Math.floor(Math.random() * 3 * jumlah)}`.trim()
                    let ud = (_ud * 1)
                    let ue = (_ue * 1)
                    let um = (_um * 1)
                    let up = (_up * 1)
...
`.trim()
        ...
        case 'mitico':
            switch (jumlah) {
                    ...
                    let _mm = `${Math.floor(Math.random() * 200 * jumlah)}`.trim()
                    let _mmm = `${Math.floor(Math.random() * mizz * jumlah)}`.trim()
                    let _me = `${Math.floor(Math.random() * 250 * jumlah)}`.trim()
                    let _mp = `${Math.floor(Math.random() * 3 * jumlah)}`.trim()
                    let _mu = `${Math.floor(Math.random() * 3 * jumlah)}`.trim()
                    let _mc = `${Math.floor(Math.random() * 5 * jumlah)}`.trim()
                    let _ml = `${Math.floor(Math.random() * mixx * jumlah)`.trim()
                    let _md = `${Math.floor(Math.random() * 3 * jumlah)}`.trim()
                    let mm = (_mm * 1)
                    let mmm = (_mmm * 1)
                    let me = (_me * 1)
                    let mp = (_mp * 1)
...
`.trim()
        ...
        case 'legendario':
            switch (jumlah) {
                    ...
                    let _lm = `${Math.floor(Math.random() * 450 * jumlah)}`.trim()
                    let _le = `${Math.floor(Math.random() * 550 * jumlah)}`.trim()
                    let _lp = `${Math.floor(Math.random() * 5 * jumlah)}`.trim()
                    let _lu = `${Math.floor(Math.random() * 7 * jumlah)}`.trim()
                    let _lc = `${Math.floor(Math.random() * 10 * jumlah)}`.trim()
                    let _ll = `${Math.floor(Math.random() * lexx * jumlah}`.trim()
                    let _lpp = `${Math.floor(Math.random() * lezz * jumlah}`.trim()
                    let _ld = `${Math.floor(Math.random() * 5 * jumlah)}`.trim()
                    let _lmm = `${Math.floor(Math.random() * leyy * jumlah}`.trim()
                    let lm = (_lm * 1)
                    let le = (_le * 1)
                    let lp = (_lp * 1) 
                    let lu = (_lu * 1) 
